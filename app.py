...
def add_trend_indicators(df):
    ...
    # Ensure each individual year column is object dtype BEFORE inserting strings
    for year in sorted_years:
        result_df[year] = result_df[year].astype("object")
    ...
    for i, row in df.iterrows():
        ...
        first_value = row[first_year]

        try:
            if pd.isna(first_value):
                formatted_value = "N/A"
            elif ratio == 'EBITDA':
                formatted_first = f"{first_value:,.2f}"
            else:
                formatted_first = f"{first_value:.2f}"
        ...
        result_df.at[i, first_year] = formatted_first

    # Use .map instead of deprecated .applymap
    result_df = result_df.style.map(style_symbols)    
            ...
            if trend_data:
                ...
                # Best and worst performing metrics
                    if not trend_df.empty:
                        safe_trend_df = trend_df.copy()

                        # Ensure 'Change %' is numeric
                        safe_trend_df["Change %"] = pd.to_numeric(safe_trend_df["Change %"], errors='coerce').fillna(0.0)

                        if not safe_trend_df["Change %"].isna().all():

                            best_metric = safe_trend_df.loc[safe_trend_df["Change %"].idxmax()]["Ratio/Calculation"]
                            worst_metric = safe_trend_df.loc[safe_trend_df["Change %"].idxmin()]["Ratio/Calculation"]
                        
                            st.success(f"**Best Performing Metric:** {best_metric}")
                            st.error(f"**Metric Requiring Most Attention:** {worst_metric}")

                        else:
                            st.info("No valid data available for trend comparison.")    
        else:
...
def load_json_in_session(json_data):
    st.session_state.financial_data = [
        {"current": json_data.get("Current", {}).get("data", {})},
        {"projected": json_data.get("Projected", {}).get("data", {})}
    ]
    st.session_state.financial_years = {
        "current_year": json_data.get("Current", {}).get("year", "N/A"),
        "projected_year": json_data.get("Projected", {}).get("year", "N/A")
    }   

def input_data():
    # File uploader to optionally load JSON
    with st.expander("Upload Financial JSON", expanded=False):
        uploaded_file = st.file_uploader("Uploaded JSON", type=["json"])
        if uploaded_file:
            try:
                json_data = json.load(uploaded_file)
                load_json_in_session(json_data)
    ...
    with st.expander("Input Financial Data", expanded=st.session_state.get("input_expanded", True)):
        ...
        if 'financial_data' not in st.session_state:
            ...
            # Ensure 'current' and 'projected' keys exist to avoid KeyErrors
            if "current" not in st.session_state.financial_data[0]:
                st.session_state.financial_data[0]["current"] = {}

            if "projected" not in st.session_state.financial_data[1]:
                st.session_state.financial_data[1]["projected"] = {}    
        ...
        with tab1:        
            ...
            current_year = st.session_state.get("financial_years", {}).get("current_year", "N/A")
            projected_year = st.session_state.get("financial_years", {}).get("projected_year", "N/A")
            st.caption(f"Current Year: {current_year}")
            st.caption(f"Projected Year: {projected_year}")
        ...
        with tab2:
            ...
            df = pd.DataFrame(df_data).fillna(0.0)
...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_file is not None:
            ...
            # Save a copy of the PDF file in memory
            ...
            # Convert BytesIO to Markdown
            dc.pdf_to_md(file_bytes, file_name)

            # Extract structured financial data as a dict
            extracted_data = dc.extract_data_to_dict(file_name)

            print(extracted_data)

            # Save JSON file
            dc.extract_dict_to_json(file_name)
